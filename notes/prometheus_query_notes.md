...
# result type is “Matrix”, rather than a single value (Vector)
...
# A more complex one is “container_cpu_usage_seconds_total”
- many pods reporting this data, so lots of series for that measurement
- constrain it using the labels, for example - to limit to the namespace “monitoring”:
...
# note - presumes “pod_name” is a label available on the container_cpu_usage_seconds_total time-series
sum by(pod_name) (
...
http://localhost:9090/api/v1/series?match[]={job="zookeeper_exporter"}

# theoretically - get a list of all time series that are available
...
Default prometheus instance (with Helm) metrics:
